...
from . import io, vulnerabilidade
...
def _tendencia_estacao(proxy: pd.DataFrame) -> pd.DataFrame:
    ...
    for estacao, g in proxy.groupby("estacao"):
        ...
        if len(g) < 5:
            ...
            coef = np.polyfit(g["ano"], g["proxy"], 1)
            slope = coef[0]
            proj = np.polyval(coef, ANO_ALVO)
...
def projetar(df_scores: pd.DataFrame) -> pd.DataFrame:
    ...
    destino = vulnerabilidade.centroides_municipios().merge(df_scores[["codigo"]], on="codigo", how="right")
    interp = vulnerabilidade.idw(tend[["slope", "delta"]], origem, destino)

    df = df_scores.merge(interp, on="codigo", how="left")
    delta_norm = vulnerabilidade.normalizar_robusto(df["delta"]) - 0.5  # centrado em 0
